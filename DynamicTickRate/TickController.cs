...
{
    // Hard caps
    public int MinTickRate = 30;   // recommended floor for headless
    public int MaxTickRate = 144;  // safe ceiling; raise if you have CPU headroom

    // Linear-ish parts
    public float AddedTicksPerUser = 0.6f;       // each non-host user (weighted) contributes this
    ...
    // Busy-world shaping
    public int   TopKWorlds = 2;           // focus on top 2 busiest worlds
...
public sealed class TickController
{
    ...
    private void Recompute_NoLock()
    {
        ...
        // If no active worlds: sink to MinTickRate and ignore stale join surges
        if (perWorldActive.Count == 0)
        {
            ...
            int candidateIdle = (int)Math.Round(MathX.Clamp(emaTick, T.MinTickRate, T.MaxTickRate));

            // avoid spammy writes, respect cooldown/hysteresis
        ...
        // Only consider ACTIVE worlds from here
